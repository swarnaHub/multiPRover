...
from model_prover import RobertaForRRWithNodeEdgeLoss
from utils_prover import (compute_metrics,
...
def evaluate(args, model, tokenizer, processor, prefix="", eval_split=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        normalized_node_logits = softmax(node_preds, axis=2)
        node_pred_logits = normalized_node_logits[:, :, 1]
        ...
        # prediction node logits
        output_node_logit_file = os.path.join(eval_output_dir, "prediction_node_logits_{}.lst".format(eval_split))
        with open(output_node_logit_file, "w") as writer:
            logger.info("***** Write predictions {} on {} *****".format(prefix, eval_split))
            for node_gold, node_pred_logit in zip(out_node_label_ids, node_pred_logits):
                node_gold = node_gold[np.where(node_gold != -100)[0]]
                node_pred_logit = node_pred_logit[:len(node_gold)]
                writer.write(str(list(node_pred_logit)) + "\n")
