...
from model_multilabel_mprover import RobertaForRRMMultilabelMprover

from utils_multilabel_mprover import (compute_metrics,
...
MODEL_CLASSES = {
    'roberta_rr': (RobertaConfig, RobertaForRRMMultilabelMprover, RobertaTokenizer)
}
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss, qa_loss, proof_loss = outputs[:3]  # model outputs are always tuple in pytorch-transformers (see doc)

            if args.n_gpu > 1:
                loss = loss.mean()  # mean() to average on multi-gpu parallel training
                # logger.info("Loss = %f", loss)
                # logger.info("QA Loss = %f", qa_loss.mean())
                # logger.info("Proof Loss = %f", proof_loss.mean())
...
def evaluate(args, model, tokenizer, processor, prefix="", eval_split=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating", mininterval=10, ncols=100):
            ...
            with torch.no_grad():
                inputs = {'input_ids': batch[0],
                          ...
                          'proof_count': batch[7]}
                outputs = model(**inputs)
                tmp_eval_loss, tmp_qa_loss, tmp_proof_loss, logits, node_logits, edge_logits = outputs[:6]
        ...
        node_preds = np.where(expit(node_preds) > 0.5, 1, 0)
        edge_pred_logits = expit(edge_preds)
        ...
        with open(output_node_pred_file, "w") as writer:
            ...
            for node_gold, node_pred in zip(out_node_label_ids, node_preds):
                node_length = len(node_gold[:, 0][np.where(node_gold[:, 0] != -100)[0]])
                sample_predicted_graph_ids = []
                for i in range(args.num_proof):
                    # Insert the node sequence only if it is not an empty graph
                    if np.any(node_pred[:node_length, i]):
                        sample_predicted_graph_ids.append(i)
                        writer.write(str(list(node_pred[:node_length, i])) + "\n")
                # If no such node sequence is found, add an empty sequence
                if len(sample_predicted_graph_ids) == 0:
                    sample_predicted_graph_ids.append(0)
                    writer.write(str([0]*node_length) + "\n")
                predicted_graph_ids.append(sample_predicted_graph_ids)
                writer.write("\n")
        ...
        with open(output_edge_pred_file, "w") as writer:
            logger.info("***** Write predictions {} on {} *****".format(prefix, eval_split))
            for (i, edge_pred_logit) in enumerate(edge_pred_logits):
                for j in predicted_graph_ids[i]:
                    writer.write(str(list(edge_pred_logit[:, j])) + "\n")
                writer.write("\n")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_split="train"):
    ...
    processor.set_num_proof(args.num_proof)
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features_RR(examples, label_list, args.max_seq_length, args.max_node_length,
    ...
    all_proof_count = torch.tensor([f.proof_count for f in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_proof_offset, all_node_label,
                            all_edge_label,
                            all_label_ids, all_proof_count)
...
def main():
    ...
    parser.add_argument("--num_proof", default=3, type=int,
                        help="Maximum number of proofs+1 (for eos)")
    ...
    model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                        config=config, num_proof=args.num_proof)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = model_class.from_pretrained(args.output_dir, num_proof=args.num_proof)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint, num_proof=args.num_proof)
            model.to(args.device)
