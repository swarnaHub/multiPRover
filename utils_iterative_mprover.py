...
class RRInputExample(object):
    def __init__(self, id, context, question, node_label, edge_label, label, proof_count):
        self.id = id
        self.context = context
        self.question = question
...
class RRProcessor(DataProcessor):
    ...
    def set_max_proof(self, max_proof):
        self.max_proof = max_proof
    ...
    def _get_node_edge_label_constrained(self, proofs, sentence_scramble, nfact, nrule):
        ...
        all_node_label = np.zeros((self.max_proof, nfact + nrule + 1), dtype=int)
        all_edge_label = np.zeros((self.max_proof, nfact + nrule + 1, nfact + nrule + 1), dtype=int)

        gold_proof_count = 0
        for (k, proof) in enumerate(proofs.split("OR")):
            # print(proof)

            if k == self.max_proof - 1:
                break
        ...
        for i in range(nfact + nrule + 1):
            for j in range(nfact + nrule + 1):
                # Ignore diagonal
                if i == j:
                    for k in range(self.max_proof):
                        all_edge_label[k][i][j] = -100
                    continue

                # Ignore edges between non-nodes
                for k in range(self.max_proof):
                    if all_node_label[k][i] == 0 or all_node_label[k][j] == 0:
                        all_edge_label[k][i][j] = -100

                is_fact_start = False
                is_fact_end = False
                if i == nfact + nrule or sentence_scramble[i] <= nfact:
                    is_fact_start = True
                if j == nfact + nrule or sentence_scramble[j] <= nfact:
                    is_fact_end = True
        ...
        # Mask the top empty proofs
        # The first empty graph denotes end of proof sequence
        for k in range(gold_proof_count + 1, self.max_proof):
            all_node_label[k, :] = -100
            all_edge_label[k, :, :] = -100

        return all_node_label.tolist(), all_edge_label.reshape(self.max_proof, -1).tolist(), gold_proof_count
    ...
    def _create_examples(self, records, meta_records):
        ...
        for (i, (record, meta_record)) in enumerate(zip(records, meta_records)):
            ...
            for (j, question) in enumerate(record["questions"]):
                ...
                node_label, edge_label, gold_proof_count = self._get_node_edge_label_constrained(proofs,
                                                                                                 sentence_scramble,
                                                                                                 nfact, nrule)

                examples.append(RRInputExample(id, context, question, node_label, edge_label, label, gold_proof_count))
...
def convert_examples_to_features_RR(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        masked_node_label, masked_edge_label = [], []
        for i in range(len(example.node_label)):
            masked_node_label.append(example.node_label[i] + [-100] * (max_node_length - len(example.node_label[i])))
            masked_edge_label.append(example.edge_label[i] + [-100] * (max_edge_length - len(example.edge_label[i])))

        assert len(input_ids) == max_seq_length
        assert len(input_mask) == max_seq_length
        assert len(segment_ids) == max_seq_length
        assert len(proof_offset) == max_node_length
        assert len(masked_node_label[0]) == max_node_length
        assert len(masked_edge_label[0]) == max_edge_length

        label_id = label_map[example.label]
        proof_count = example.proof_count
        ...
        if ex_index < 5:
            ...
            logger.info("node_label: %s" % " ".join([str(x) for x in masked_node_label[0]]))
            logger.info("edge_label: %s" % " ".join([str(x) for x in masked_edge_label[0]]))
            logger.info("label: %s (id = %d)" % (example.label, label_id))
            logger.info("proof_count: %s" % proof_count)
        ...
        features.append(
            RRFeatures(id=id,
                       ...
                       node_label=masked_node_label,
                       edge_label=masked_edge_label,
                       label_id=label_id,
