...
from model_iterative_mprover import RobertaForRRIterativeMprover
from utils_iterative_mprover import (compute_metrics,
...
MODEL_CLASSES = {
    'roberta_rr': (RobertaConfig, RobertaForRRIterativeMprover, RobertaTokenizer)
}
...
def evaluate(args, model, tokenizer, processor, prefix="", eval_split=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        node_preds = np.argmax(node_preds, axis=3)

        normalized_logits = softmax(edge_preds, axis=3)
        edge_pred_logits = normalized_logits[:, :, :, 1]
        edge_preds = np.argmax(edge_preds, axis=3)
        ...
        with open(output_node_pred_file, "w") as writer:
            ...
            for node_gold, node_pred in zip(out_node_label_ids, node_preds):
                ...
                count = 0
                for curr_node_gold, curr_node_pred in zip(node_gold, node_pred):
                    curr_node_pred = curr_node_pred[:node_length]
                    # If eos is reached (empty graph) and it's not the first prediction, we stop
                    if not np.any(curr_node_pred):
                        if count > 0:
                            break
                        else:
                            count += 1
                            writer.write(str(list(curr_node_pred)) + "\n")
                    else:
                        count += 1
                        writer.write(str(list(curr_node_pred)) + "\n")
                predicted_graph_counts.append(count)
                writer.write("\n")
        ...
        with open(output_edge_pred_file, "w") as writer:
            ...
            for (i, edge_pred_logit) in enumerate(edge_pred_logits):
                # Consider only up to the number of predicted node set
                # since edge module is conditioned on node module
                for j in range(predicted_graph_counts[i]):
                    curr_edge_pred_logit = edge_pred_logit[j]
                    writer.write(str(list(curr_edge_pred_logit)) + "\n")
                writer.write("\n")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_split="train"):
    ...
    processor.set_max_proof(args.max_proof)
...
def main():
    ...
    parser.add_argument("--max_proof", default=4, type=int,
    ...
    model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                        config=config, max_proof=args.max_proof)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = model_class.from_pretrained(args.output_dir, max_proof=args.max_proof)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint, max_proof=args.max_proof)
            model.to(args.device)
