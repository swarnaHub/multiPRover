...
class RRProcessor(DataProcessor):
    ...
    def set_num_proof(self, num_proof):
    ...
    def _get_node_edge_label_constrained(self, proofs, sentence_scramble, nfact, nrule):
        ...
        all_node_label = np.zeros((nfact + nrule + 1, self.num_proof), dtype=int)
        all_edge_label = np.zeros((nfact + nrule + 1, nfact + nrule + 1, self.num_proof), dtype=int)
        ...
        for (k, proof) in enumerate(proofs.split("OR")):
            ...
            for node in nodes:
                index = component_index_map[node]
                all_node_label[index][k] = 1

            edges = list(set(edges))
            for edge in edges:
                start_index = component_index_map[edge[0]]
                end_index = component_index_map[edge[1]]
                all_edge_label[start_index][end_index][k] = 1
        ...
        for i in range(nfact + nrule + 1):
            for j in range(nfact + nrule + 1):
                # Ignore diagonal
                if i == j:
                    for k in range(self.num_proof):
                        all_edge_label[i][j][k] = -100
                    continue

                # Ignore edges between non-nodes
                for k in range(self.num_proof):
                    if all_node_label[i][k] == 0 or all_node_label[j][k] == 0:
                        all_edge_label[i][j][k] = -100
        ...
        return all_node_label, all_edge_label.reshape(-1, self.num_proof), gold_proof_count
...
def convert_examples_to_features_RR(examples,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        num_proof = example.node_label.shape[1]
        masked_node_label, masked_edge_label = np.ones((max_node_length, num_proof), dtype=int)*-100, \
                                               np.ones((max_edge_length, num_proof), dtype=int)*-100
        for i in range(num_proof):
            masked_node_label[:len(example.node_label[:, i]), i] = example.node_label[:, i]
            masked_edge_label[:len(example.edge_label[:, i]), i] = example.edge_label[:, i]
        ...
        features.append(
            RRFeatures(id=id,
                       ...
                       node_label=masked_node_label.tolist(),
                       edge_label=masked_edge_label.tolist(),
                       label_id=label_id,
